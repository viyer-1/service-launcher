...
#
# Usage:
#   bash install-mac.sh              # installs for the current user
#   bash install-mac.sh --uninstall  # removes the service
#   bash install-mac.sh --port=8080  # install on a custom port
...
APP_DIR="$SCRIPT_DIR/v3"

INSTALL_USER="$(whoami)"
INSTALL_HOME="$HOME"

VENV_DIR="$INSTALL_HOME/.venvs/main"
...
for arg in "$@"; do
    case $arg in
        ...
        --port=*)       PORT="${arg#*=}" ;;
        --help|-h)
            echo "Usage: bash install-mac.sh [--uninstall] [--port=5000]"
            exit 0
            ;;
    esac
done
...
if [[ ! -d "$APP_DIR" ]]; then
    error "v3/ directory not found at $APP_DIR"
...
echo ""
echo "  To uninstall: bash install-mac.sh --uninstall"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
