...
#
# Usage:
#   bash installation/install.sh              # installs for the current user
#   sudo bash installation/install.sh         # same (uses SUDO_USER if available)
#   bash installation/install.sh --uninstall  # removes the service
...
for arg in "$@"; do
    case $arg in
        ...
        --port=*)       PORT="${arg#*=}" ;;
        --help|-h)
            echo "Usage: bash installation/install.sh [--uninstall] [--port=5000]"
            exit 0
            ;;
    esac
done
...
if ! command -v systemctl &>/dev/null; then
    ...
    error "To run manually instead: ./start.sh"
    exit 1
fi
...
if ! command -v sudo &>/dev/null; then
    ...
    error "Install sudo or run as root with: su -c 'bash installation/install.sh'"
    exit 1
fi
...
[Unit]
Description=Service Launcher - Web Dashboard
...
SyslogIdentifier=service-launcher
OOMScoreAdjust=-100
...
echo ""
echo "  To uninstall: bash installation/install.sh --uninstall"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
